...
把网易云音乐 **macOS 版**的实时播放进度和逐字歌词，通过 HTTP / WebSocket 暴露出来。自带歌词磁盘缓存，和一个可以直接丢进 OBS 的透明歌词页。
...
`--remote-debugging-port=9222` 等于把网易云的渲染进程完全暴露给 localhost，任何本机进程都能在里面执行 JS、读你的登录态。9222 和 3574 都只监听回环地址，别往外放。这是这套方案的固有代价。
